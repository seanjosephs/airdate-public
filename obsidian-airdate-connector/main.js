...
class PairModal extends Modal {
  constructor(app, plugin) {
    ...
    this.values = {
      ...
      port: String(plugin.settings.port || DEFAULT_PORT),
    };
  }
...
class AirdateConnector extends Plugin {
  ...
  async pair(values) {
    ...
    const port = parsePort(values.port === undefined ? this.settings.port : values.port);
    if (!port) return { ok: false, message: "Port must be a number from 1024 to 65535." };
    const portChanged = port !== this.settings.port;
    const token = crypto.randomBytes(32).toString("hex");
    try {
      writePairingFile(dataDir, port, token);
    } catch (error) {
